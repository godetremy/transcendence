...
import { getMemberById, getOrganizationMemberByFilter } from "@/database/OrganizationMembers";
...
export async function PATCH(req: NextRequest, { params }: { params: Promise<{ album_id: string }> }): Promise<NextResponse> {
	return errorHandler(async () => {
		const {album_id} = await params;
		const session = await getThrowableSession(req);
		...
		if (user.admin == false && checkAlbum.events?.organization.owner_id != user.id && checkAlbum.services?.organization.owner_id != user.id) {
			if (checkAlbum.events?.organization_id != null) {
				const member = await getOrganizationMemberByFilter({ organization_id: checkAlbum.events?.organization_id, user_id: user.id }, {});
				if (member == null || member.permission_id == null) throw ERRORS_DETAILS.member_not_in_organization();
				const permissison = await getOrganizationPermissionById(member.permission_id, checkAlbum.events?.organization_id, {});
				...
				const member = await getOrganizationMemberByFilter({ organization_id: checkAlbum.services?.organization_id, user_id: user.id }, {});
				if (member == null || member.permission_id == null) throw ERRORS_DETAILS.member_not_in_organization();
				const permissison = await getOrganizationPermissionById(member.permission_id, checkAlbum.services?.organization_id, {});
				if (permissison == null || permissison.album_update == false) throw ERRORS_DETAILS.permission_denied();
			}
