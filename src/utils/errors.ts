...
const ERRORS_DETAILS: Record<string, (...args: string[]) => ApiError> = {
	...
	two_factor_auth_required: () =>
		new ApiError(401, 'Une authentification à deux facteurs est requise pour ce compte.'),
	...
	invalid_parameter: (parameter: string) =>
		new ApiError(400, `Le paramètre suivant \'${parameter}\' est non valide.`),

	password_not_set: () =>
		new ApiError(400, 'Il semble que votre mot de passe ne soit pas défini. Êtes-vous étudiant ?'),
	...
	organization_does_not_verified: () =>
		new ApiError(400, 'Cette organisation n’est pas vérifiée par l’administrateur.'),
	...
	cant_remove_last_admin: () =>
		new ApiError(
			401,
			'Vous ne pouvez pas quitter votre rôle d’administrateur car vous êtes le dernier administrateur.'
		),
	cant_leave_as_owner: () =>
		new ApiError(403, 'Vous ne pouvez pas quitter cette organisation parce que vous en êtes le propriétaire.'),
	...
	event_does_not_register: () =>
		new ApiError(400, 'Il semblerait que vous ne soyez pas enregistré pour cet événement.'),
	...
	too_many_upload: () =>
		new ApiError(
			429,
			'Vous avez demandé trop de téléchargements. Veuillez patienter quelques heures et réessayer.'
		),
