...
const getUserOrganizationPermission = async (
	...
	if (!organization_member) throw ERRORS_DETAILS.organization_does_not_exist;

	if (user.admin || organization_member.organization.owner_id === user.id)
		return FULL_PERMISSIONS(organization_member.organization_id);
