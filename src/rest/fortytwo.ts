...
export async function getFortyTwoOauthToken(code: string): Promise<FortyTwoOauthToken> {
    const authorize_fetch: Response = await fetch(
        `${FORTY_TWO_BASE_URL}/oauth/token`,
        {
            method: "POST",
            body: JSON.stringify({
    ...
    if (!authorize_fetch.ok)
        throw new Error(`42 API repond with status code ${authorize_fetch.status}`);
    return await authorize_fetch.json();
}

export async function getFortyTwoMe(token: string): Promise<FortyTwoCursusUserDetails> {
    const me_fetch: Response = await fetch(
        `${FORTY_TWO_BASE_URL}/v2/me`,
        {
            headers: {
    ...
    if (!me_fetch.ok)
        throw new Error(`42 API repond with status code ${me_fetch.status}`);
